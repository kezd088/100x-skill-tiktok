...
公开包 v2.3 延续已验证的视频反推与严格校验能力，并把默认交付收敛为由 `digest.py` 确定性生成的中文运营审片视图。当前能力由五部分组成：

1. FFmpeg／FFprobe 本地只读证据提取。
2. 完整视频、稠密帧和音轨联合的多模态语义校正。
3. 稳定资产 ID、一致性锚点和生成提示词包。
4. JSON Schema、媒体物化、源哈希和 provenance 严格验证。
5. 从同一份 `reverse.json` 投影窄幅竖屏播放器、每段首／高光／尾三帧组、可拖 JPEG 预览、原帧路径兜底、所属分段提示词、资产和逐段生成方案；多包使用轻量总览加独立详情。
...
- 当前版本：`100x-video-reverse@2.3.0`。
- 被替换版本：`2.2.0`，可恢复提交为 `a0b16ea78c5b7ae0670a58c6d6131bb8b3fa7c76`。
- 替换原因：中文运营反馈指出默认层英文过多、图片不能直接拖出、每段首／尾／高光帧没有稳定同组；旧展示还会按镜头数量静默省略高光帧或尾帧。
- 兼容性：`2.3.0` 不修改 v2.1 机器 Schema 或 `execution_plan`。`digest.py` 同时读取 v2.1 `execution_plan` 和新版运行包的 `recommendation`；三帧、中文标签和拖拽都只属于可删除、可再生的展示层，不回写机器事实。
