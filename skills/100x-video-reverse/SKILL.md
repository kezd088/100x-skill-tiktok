...
metadata:
  ...
  supersedes: "100x-video-reverse@2.1.0"
---

# 100X Video Reverse v2.2
...
7. 严格验证通过后运行 `scripts/digest.py`，再按 [native-output.md](references/native-output.md) 原样交付。Codex 使用 `--format fragment --fragment-dir <本轮 thread 可视化目录>`：首 tab 是左侧窄 9:16 播放器 + 右侧多列帧板；点击镜头会暂停并定位视频，同时显示该镜头所属生成分段的完整提示词，播放跨镜头时同步更新。窄屏改为上下结构和横向帧轨。其他客户端默认使用 `--format md`；判断不了客户端能力时不得赌内联协议。
...
完成时必须存在：`source_manifest.json`、`evidence/evidence_manifest.json`、证据媒体、`reverse.json`、`materialization_manifest.json` 和 `validation.json`；严格媒体验证为零硬错误。Codex fragment 的首 tab 必须展示完整时长预览和全部镜头帧索引，桌面端使用窄播放器列 + 宽多列帧板，点击镜头能定位并查看所属分段提示词；其余五块随后展开。多包必须先给轻量批次总览，再按编号给每条独立详情，失败条目不能消失。机器包路径仍须在正文给出。`--legacy-unverified-media` 只允许审计历史包，禁止用于新交接。
